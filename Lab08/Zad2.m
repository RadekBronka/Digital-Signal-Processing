...
% Wybierz jedną stację z każdej transmisji:
% DSB-C (fc1)
r = yDSBC .* cos(2*pi*fc1*t);
demod1 = lowpass(r, 4e3, fs); % filtracja

% DSB-SC (fc1)
r = yDSBSC .* cos(2*pi*fc1*t);
demod2 = lowpass(r, 4e3, fs);

% SSB-SC (fc1, LSB)
r = ySSBSC .* (cos(2*pi*fc1*t) + 1i*sin(2*pi*fc1*t));
demod3 = real(lowpass(r, 4e3, fs));

% Konwersja do fsx
demod1 = resample(demod1, fsx, fs);
demod2 = resample(demod2, fsx, fs);
demod3 = resample(demod3, fsx, fs);




% x1 - LSB, x2 - USB na tej samej nośnej fc = 100kHz
y1 = 0.5 * x1u .* cos(2*pi*fc1*t) - 0.5 * x1H .* sin(2*pi*fc1*t); % LSB
y2 = 0.5 * x2u .* cos(2*pi*fc1*t) + 0.5 * x2H .* sin(2*pi*fc1*t); % USB
ySSB_SAMECARRIER = y1 + y2;

% Demodulacja LSB (x1)
r = ySSB_SAMECARRIER .* (cos(2*pi*fc1*t) - 1i*sin(2*pi*fc1*t));
demod_LSB = real(lowpass(r, 4e3, fs));
%soundsc(resample(demod_LSB, fsx, fs), fsx);
