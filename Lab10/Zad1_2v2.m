...
% === Widmo LPC ===
[H, f_lpc] = freqz(1, a, 512, fs_target);

% === Rysowanie widma LPC ===
figure('Name', sprintf('Widmo LPC - %s, p=%d', labels{idx}, p));
plot(f_lpc, 20*log10(abs(H)), 'LineWidth', 2);
xlabel('Częstotliwość [Hz]');
ylabel('Amplituda [dB]');
title(sprintf('|H(f)| dla %s (p=%d)', labels{idx}, p));
grid on;
